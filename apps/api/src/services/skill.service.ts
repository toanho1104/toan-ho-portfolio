...
export const skillService = {
  ...
  async getAllCategories(userId?: string) {
    return db.query.skillCategories.findMany({
      where: userId ? eq(skillCategories.userId, userId) : undefined,
  ...
  async createCategory(
    userId: string,
    data: Partial<typeof skillCategories.$inferInsert>,
  ) {
  ...
  async getAll(filters: SkillQuery = {}) {
    ...
    const where = filters.categoryId
      ? eq(skills.categoryId, filters.categoryId)
      : undefined;
    ...
    return {
      data,
      meta: buildPaginationMeta(countResult[0]?.total ?? 0, page, limit),
    };
  },
  ...
  async create(data: typeof createSkillBodyValidator.$inferInsert) {
  ...
  async update(
    id: string,
    data: Partial<typeof updateSkillBodyValidator.$inferInsert>,
  ) {
    const [updated] = await db
      .update(skills)
      .set(data)
